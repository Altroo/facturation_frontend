...
jest.mock('@/utils/hooks', () => ({
	...
	useAppSelector: jest.fn(() => [
		{ id: 1, name: 'Chèque', can_change_document_status: true },
		{ id: 2, name: 'Virement' },
	]),
	useLanguage: () => ({
...
const mockQueryData = {
	results: [
		{
			id: 1,
			facture_client_numero: 'FAC-001',
			client: 10,
			client_name: 'Client Alpha',
			mode_reglement_name: 'Chèque',
			montant: 5000,
			devise: 'MAD',
		...
		{
			id: 2,
			facture_client_numero: 'FAC-002',
			client: 20,
			client_name: 'Client Beta',
			mode_reglement_name: null,
			montant: 3000,
			devise: 'EUR',
			date_reglement: null,
			date_echeance: null,
...
jest.mock('@/utils/helpers', () => ({
	...
	formatNumberWithSpaces: (n: number, d: number) =>
		n != null ? n.toFixed(d).replace(/\B(?=(\d{3})+(?!\d))/g, ' ') : '0.00',
...
jest.mock('@/utils/routes', () => ({
	...
	REGLEMENT_PDF: (id: number, companyId: number, lang: string) =>
		`/api/reglement/${id}/pdf?company=${companyId}&lang=${lang}`,
}));
import ReglementListClient from './reglement-list';
import { reglementStatusFilterOptions } from '@/utils/rawData';
...
describe('ReglementListClient', () => {
	...
	describe('Rendering', () => {
		...
		it('renders Nouveau règlement button', () => {
			render(<ReglementListClient session={mockSession} />);
			expect(screen.getByText('Nouveau règlement')).toBeInTheDocument();
		});
	...
	describe('Action handlers', () => {
		...
		it('handles delete error', async () => {
			...
			await waitFor(() => {
				expect(mockOnError).toHaveBeenCalledWith('Erreur lors de la suppression du règlement');
			});
		});
		...
		it('cancels reglement on confirm', async () => {
			...
			await act(async () => {
				fireEvent.click(screen.getByText('Annuler le règlement'));
			});
			await waitFor(() => {
		...
		it('handles cancel error', async () => {
			...
			await waitFor(() => {
				expect(mockOnError).toHaveBeenCalledWith("Erreur lors de l'annulation du règlement");
			});
		});
	...
	describe('PDF print flow', () => {
		it('opens language modal on print click', async () => {
			render(<ReglementListClient session={mockSession} />);
			await act(async () => {
				fireEvent.click(screen.getAllByText('Afficher le reçu de règlement')[0]);
			});
	...
	describe('Loading and empty states', () => {
		it('renders when loading', () => {
			mockUseGetReglementsListQuery.mockReturnValueOnce({
				data: {
					results: [],
					count: 0,
					next: null,
					previous: null,
					stats_by_currency: {
						MAD: { chiffre_affaire_total: 0, total_reglements: 0, total_impayes: 0 },
						EUR: { chiffre_affaire_total: 0, total_reglements: 0, total_impayes: 0 },
					},
				},
				isLoading: true,
				refetch: mockRefetch,
	...
	describe('Exports', () => {
		it('defines the payment status options', () => {
			expect(reglementStatusFilterOptions).toHaveLength(2);
			expect(reglementStatusFilterOptions[0].value).toBe('Valide');
			expect(reglementStatusFilterOptions[1].value).toBe('Annulé');
		});
	});
});
