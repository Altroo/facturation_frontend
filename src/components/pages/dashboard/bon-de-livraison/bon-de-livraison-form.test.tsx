...
jest.mock('next/navigation', () => ({
	...
	usePathname: () => '/dashboard/bon-de-livraison/new',
}));
...
// Mock bon de livraison service hooks
const mockUseGetBonDeLivraisonQuery = jest.fn();
const mockUseGetNumBonDeLivraisonQuery = jest.fn();
const mockAddBonDeLivraisonMutation = jest.fn();
const mockEditBonDeLivraisonMutation = jest.fn();
...
describe('BonDeLivraisonForm', () => {
	beforeEach(() => {
		jest.clearAllMocks();
		mockUseGetBonDeLivraisonQuery.mockReturnValue({
			data: undefined,
			isLoading: false,
			error: undefined,
		});
		mockUseGetNumBonDeLivraisonQuery.mockReturnValue({
			data: { numero_bon_livraison: '001/25' },
			isLoading: false,
		});
	});
	...
	describe('Add Mode (no id)', () => {
		it('renders wrapper and form content in add mode', () => {
			renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={123} />);
		...
		it('passes correct props in add mode', () => {
			renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={456} />);
		...
		it('passes token to form content', () => {
			renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={789} />);
	...
	describe('Edit Mode (with id)', () => {
		it('renders in edit mode when id is provided', () => {
			renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={123} id={999} />);
		...
		it('passes correct company_id in edit mode', () => {
			renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={555} id={111} />);
	...
	describe('Hook calls', () => {
		it('calls useGetBonDeLivraisonQuery (hook invoked) when in edit mode', () => {
			renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={123} id={1} />);

			expect(mockUseGetBonDeLivraisonQuery).toHaveBeenCalled();
		});

		it('calls useGetNumBonDeLivraisonQuery in add mode', () => {
			renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={123} />);

			expect(mockUseGetNumBonDeLivraisonQuery).toHaveBeenCalled();
		});
	});
});

describe('BonDeLivraisonForm Configuration', () => {
	it('renders without errors and uses form content', () => {
		renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={1} />);
		expect(screen.getByTestId('company-document-form-content')).toBeInTheDocument();
	});

	it('renders in edit mode with id and uses form content', () => {
		renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={1} id={1} />);
		expect(screen.getByTestId('company-document-form-content')).toBeInTheDocument();
	});
});

describe('BonDeLivraisonForm mutation wrappers', () => {
	beforeEach(() => {
		jest.clearAllMocks();
		mockUseGetBonDeLivraisonQuery.mockReturnValue({ data: undefined, isLoading: false, error: undefined });
		mockUseGetNumBonDeLivraisonQuery.mockReturnValue({ data: { numero_bon_livraison: 'BL-001/25' }, isLoading: false });
	});

	it('addData wrapper calls addBonDeLivraisonMutation', () => {
		mockAddBonDeLivraisonMutation.mockReturnValue({ unwrap: jest.fn().mockResolvedValue({ id: 1 }) });
		renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={1} />);
		screen.getByTestId('call-add').click();
		expect(mockAddBonDeLivraisonMutation).toHaveBeenCalled();
	});

	it('updateData wrapper calls editBonDeLivraisonMutation', () => {
		mockEditBonDeLivraisonMutation.mockReturnValue({ unwrap: jest.fn().mockResolvedValue({}) });
		renderWithProviders(<BonDeLivraisonForm session={mockSession} company_id={1} id={1} />);
		screen.getByTestId('call-update').click();
		expect(mockEditBonDeLivraisonMutation).toHaveBeenCalled();
	});
