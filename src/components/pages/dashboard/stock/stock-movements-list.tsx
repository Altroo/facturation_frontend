...
import { stockMovementOptions } from '@/utils/rawData';
...
const StockMovementsContent: FC<{ company_id: number }> = ({ company_id }) => {
	...
	const chipFilters = [
		...
		{ key: 'type', label: 'Type de mouvement', paramName: 'movement_types', options: stockMovementOptions },
	] as ChipFilterConfig[];
	...
	const columns = [
		...
		},
		{
			...
			renderCell: (params: GridRenderCellParams<StockMovement>) => (
				<DarkTooltip title={params.row.article_designation}>
					<Typography variant="body2" noWrap>
						{params.row.article_designation}
					</Typography>
				</DarkTooltip>
			),
		},
		{ field: 'emplacement_name', headerName: 'Emplacement', minWidth: 135, flex: 0.9 },
		{
			...
			minWidth: 120,
			flex: 0.8,
			filterOperators: createDropdownFilterOperators(stockMovementOptions, 'Tous les types'),
			renderCell: (params: GridRenderCellParams<StockMovement>) => (
				<Chip
					size="small"
					variant="outlined"
...
const StockMovementsListClient: FC<SessionProps> = ({ session }) => {
