...
import { stockInventoryStatusOptions } from '@/utils/rawData';

const inventoryStatusLabel = (status: InventorySession['status']) =>
	stockInventoryStatusOptions.find((option) => option.value === status)?.label ?? status;
...
const StockInventoriesContent: FC<{ company_id: number; role: string }> = ({ company_id, role }) => {
	...
	const chipFilters = [
		...
		{ key: 'status', label: 'Statut', paramName: 'statuses', options: stockInventoryStatusOptions },
	] as ChipFilterConfig[];
	...
	const columns = [
		...
		},
		{
			...
			minWidth: 115,
			flex: 0.9,
			filterOperators: createDropdownFilterOperators(stockInventoryStatusOptions, 'Tous les statuts', true),
			renderCell: (params: GridRenderCellParams<InventorySession>) => (
				<Chip
					size="small"
					variant="outlined"
			...
			{canAdjust && (
				...
				>
					<Button
						variant="contained"
						startIcon={<AddIcon />}
						onClick={() => router.push(STOCK_INVENTORIES_ADD(company_id))}
					>
						Nouvel inventaire
					</Button>
				</Box>
...
const StockInventoriesListClient: FC<SessionProps> = ({ session }) => {
