...
import { stockStateOptions } from '@/utils/rawData';
...
const StockListContent: FC<{ company_id: number; role: string }> = ({ company_id, role }) => {
	...
	const columns = [
		...
		},
		{
			...
			renderCell: (params: GridRenderCellParams<StockBalance>) =>
				quantityText(params.row.physical_quantity, 'primary.main'),
		},
		{
			...
			renderCell: (params: GridRenderCellParams<StockBalance>) =>
				quantityText(
					params.row.reserved_quantity,
					Number(params.row.reserved_quantity) > 0 ? 'warning.main' : 'text.secondary',
				),
		},
		{
		...
		},
		{
			...
			renderCell: (params: GridRenderCellParams<StockBalance>) =>
				quantityText(
					params.row.incoming_quantity,
					Number(params.row.incoming_quantity) > 0 ? 'info.main' : 'text.secondary',
				),
		},
		{
			...
			valueGetter: (value: string | number | null | undefined) => Number(value ?? 0),
			renderCell: (params: GridRenderCellParams<StockBalance>) =>
				quantityText(params.row.projected_quantity, 'primary.main'),
		},
		{
			field: 'stock_state',
			headerName: 'État',
			...
			renderCell: (params: GridRenderCellParams<StockBalance>) => (
				<DarkTooltip title={stateLabel(params.row.stock_state)}>
					<Chip
						size="small"
						variant="outlined"
						label={stateLabel(params.row.stock_state)}
						color={stateColor(params.row.stock_state)}
					/>
				</DarkTooltip>
			),
			...
			{canAdjust && (
				...
				>
					<Button
						...
						onClick={() => router.push(STOCK_ADD(company_id))}
					>
						Ajuster le stock
					</Button>
				</Box>
...
const StockListClient: FC<SessionProps> = ({ session }) => {
