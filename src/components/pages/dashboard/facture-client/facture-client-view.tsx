...
const FactureClientViewClient: React.FC<Props> = ({ session, company_id, id }) => {
	...
	const { data: rawData, isLoading, error } = useGetFactureClientQuery({ id }, { skip: !token });
	...
	return (
		<Stack direction="column" spacing={2} className={Styles.flexRootStack} mt="32px">
			<NavigationBar title="Détails du facture client">
					...
					<Stack direction={isMobile ? 'column' : 'row'} justifyContent="space-between" spacing={2}>
						...
						{!isLoading && !error && company?.role === 'Admin' && (
							<Button
								variant="contained"
								startIcon={<EditIcon />}
								onClick={() => router.push(FACTURE_CLIENT_EDIT(id, company_id))}
								sx={{ width: isMobile ? '100%' : 'auto' }}
							>
								Modifier
							</Button>
