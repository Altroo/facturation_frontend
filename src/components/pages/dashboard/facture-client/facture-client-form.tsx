...
import { CLIENTS_ADD, FACTURE_CLIENT_LIST, FACTURE_CLIENT_EDIT } from '@/utils/routes';
...
const FormikContent: React.FC<FormikContentProps> = (props: FormikContentProps) => {
	...
	const [addData, { isLoading: isAddLoading, error: addError }] = useAddFactureClientMutation();
	const [updateData, { isLoading: isUpdateLoading, error: updateError }] = useEditFactureClientMutation();
		...
		onSubmit: async (data, { setFieldError }) => {
			...
			try {
				if (isEditMode) {
					...
					onSuccess('Facture client ajouté avec succès.');
					if (response.id) {
						setTimeout(() => {
							router.replace(FACTURE_CLIENT_EDIT(response.id, company_id));
						}, 500);
					}
				}
			} catch (e) {
				...
				onError(
					isEditMode
						? 'Échec de la mise à jour de facture client. Veuillez réessayer.'
						: "Échec de l'ajout de facture client. Veuillez réessayer.",
				);
			} finally {
				setIsPending(false);
			}
		...
		<LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={fr}>
			<Stack spacing={3} sx={{ p: { xs: 2, md: 3 } }}>
				<Stack direction={isMobile ? 'column' : 'row'} pt={2} justifyContent="space-between" spacing={2}>
					<Button
						...
						onClick={() => router.push(FACTURE_CLIENT_LIST)}
						sx={{
							whiteSpace: 'nowrap',
							px: { xs: 1.5, sm: 2, md: 3 },
							py: { xs: 0.8, sm: 1, md: 1 },
				...
				{isLoading ? (
					...
					<form onSubmit={formik.handleSubmit}>
						<Stack spacing={3}>
										...
										<Stack direction="row" spacing={2} alignItems="flex-start">
											<Box sx={{ flex: 2 }}>
												<CustomTextInput
													...
													onBlur={formik.handleBlur('numero_facture')}
													error={formik.touched.numero_facture && Boolean(formik.errors.numero_facture)}
													helperText={
														formik.touched.numero_facture && formik.errors.numero_facture
															? formik.errors.numero_facture
															: ''
													}
													fullWidth={true}
													size="small"
							...
							{isEditMode && (
								<LinesGrid
									title="Lignes du facture client"
									rows={dataGridRows}
			...
			{showDeleteConfirm && (
				<ActionModals
					...
					body="Êtes-vous sûr de vouloir supprimer cette ligne du facture client ?"
