...
beforeEach(() => {
	...
	process.env.NEXT_PUBLIC_EBH_SSO_APP_SLUG = 'facturation';
	process.env.NEXT_PUBLIC_DOMAIN_URL_PREFIX = 'https://facturation.example.test';
});
...
describe('SSOStartPage', () => {
	it('redirects to central authorize with app metadata', () => {
		...
		expect(url.searchParams.get('app')).toBe('facturation');
		expect(url.searchParams.get('redirect_uri')).toBe('https://facturation.example.test/sso/callback');
