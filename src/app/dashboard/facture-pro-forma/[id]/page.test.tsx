...
jest.mock('@/components/pages/dashboard/pro-forma/pro-formaView', () => ({
	...
	default: (props: { session?: Session; id?: number; company_id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`PRO_FORMA_VIEW_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}:COMPANY_ID=${props?.company_id ?? ''}`,
		);
	},
}));
...
describe('ProFormaViewPage server component', () => {
	...
	it('renders ProForma view with session, id, and company_id when valid', async () => {
