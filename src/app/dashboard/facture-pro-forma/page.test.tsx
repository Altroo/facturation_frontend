...
jest.mock('@/components/pages/dashboard/pro-forma/pro-formaList', () => ({
	...
	default: (props: { session?: Session }) => {
		...
		return React.createElement('div', null, `PROFORMA_LIST_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}`);
	},
}));
...
describe('ProFormaListPage server component', () => {
	...
	it('renders ProformaListClient with session when session exists', async () => {
		const sessionValue: Session = { user: { pk: 303, email: 'pro@site.com' } };
		...
		expect(decoded).toContain('"pk":303');
		expect(decoded).toContain('"email":"pro@site.com"');
