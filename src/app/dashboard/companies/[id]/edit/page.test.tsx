...
// mock CompaniesForm
jest.mock('@/components/pages/dashboard/companies/companiesForm', () => ({
	__esModule: true,
	default: (props: { session?: Session; id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`COMPANIES_FORM_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}`,
		);
	},
}));
...
describe('CompaniesEditPage server component', () => {
	...
	it('renders CompaniesForm with session and id when session exists', async () => {
		const sessionValue: Session = { user: { pk: 55, email: 'company@site.com' } };
		...
		const result = await Page!({ params: Promise.resolve({ id: 789 }) });
		const html = renderToStaticMarkup(result as unknown as React.ReactElement);
		const decoded = html.replace(/&quot;/g, '"');

		expect(decoded).toContain('"pk":55');
		expect(decoded).toContain('"email":"company@site.com"');
		expect(decoded).toContain('ID=789');
