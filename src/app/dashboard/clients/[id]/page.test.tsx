...
// mock ClientsViewClient
jest.mock('@/components/pages/dashboard/clients/clients-view', () => ({
	__esModule: true,
	default: (props: { session?: Session; id?: number; company_id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`CLIENTS_VIEW_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}:COMPANY_ID=${props?.company_id ?? ''}`,
		);
	},
}));
...
describe('ClientsViewPage server component', () => {
	...
	it('renders ClientsViewClient with session, id, and company_id when valid', async () => {
