...
// mock ClientsListClient
jest.mock('@/components/pages/dashboard/clients/clients-list', () => ({
	__esModule: true,
	default: (props: { session?: Session; archived?: boolean }) => {
		return createElement(
			'div',
			null,
			`CLIENTS_LIST_MARKER:${JSON.stringify(props?.session ?? null)}:ARCHIVED=${props?.archived ?? false}`,
		);
	},
}));
...
describe('ClientsArchivedListPage server component', () => {
	...
	it('renders ClientsListClient with session and archived=true when session exists', async () => {
		const sessionValue: Session = { user: { pk: 42, email: 'archived@site.com' } };
		...
		expect(decoded).toContain('"email":"archived@site.com"');
