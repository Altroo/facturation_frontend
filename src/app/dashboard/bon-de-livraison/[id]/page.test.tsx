...
// mock BonDeLivraisonViewClient
jest.mock('@/components/pages/dashboard/bon-de-livraison/bon-de-livraison-view', () => ({
	__esModule: true,
	default: (props: { session?: Session; id?: number; company_id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`BON_DE_LIVRAISON_VIEW_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}:COMPANY_ID=${props?.company_id ?? ''}`,
		);
	},
}));
...
describe('BonDeLivraisonViewPage server component', () => {
	...
	it('renders BonDeLivraisonViewClient with session, id, and company_id when valid', async () => {
