...
jest.mock('@/components/pages/dashboard/articles/articlesView', () => ({
	...
	default: (props: { session?: Session; id?: number; company_id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`ARTICLES_VIEW_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}:COMPANY_ID=${props?.company_id ?? ''}`,
		);
	},
}));
...
describe('ArticlesViewPage server component', () => {
	...
	it('renders ArticlesViewClient with session, id, and company_id when valid', async () => {
