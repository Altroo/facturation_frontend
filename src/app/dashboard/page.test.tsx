...
jest.mock('@/components/pages/dashboard/dashboard-home/dashboard-view', () => ({
	__esModule: true,
	default: (props: { session?: Session }) => {
		// eslint-disable-next-line @typescript-eslint/no-require-imports
		const React = require('react');
		return React.createElement('div', null, `DASHBOARD_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}`);
	},
}));
...
describe('DashboardPage server component', () => {
	...
	it('renders DashboardClient with session when session exists', async () => {
		const sessionValue: Session = { user: { pk: 42, email: 'x@y.com' } };
		...
		expect(html).toContain('DASHBOARD_CLIENT_MARKER:');
		const decoded = html.replace(/&quot;/g, '"');
		expect(decoded).toContain('"pk":42');
		expect(html).toContain('&quot;pk&quot;:42');
		expect(html).toContain('&quot;email&quot;:&quot;x@y.com&quot;');
