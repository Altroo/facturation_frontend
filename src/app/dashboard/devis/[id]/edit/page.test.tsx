...
// mock DevisEditForm
jest.mock('@/components/pages/dashboard/devis/devisEditForm', () => ({
	__esModule: true,
	default: (props: { session?: Session; id?: number; company_id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`DEVIS_EDIT_FORM_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}:COMPANY_ID=${props?.company_id ?? ''}`,
		);
	},
}));
...
describe('DevisEditPage server component', () => {
	...
	it('renders DevisEditForm with session, id, and company_id when valid', async () => {
