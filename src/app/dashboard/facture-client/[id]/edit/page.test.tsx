...
// mock form component to return a renderable element
jest.mock('@/components/pages/dashboard/facture-client/facture-client-form', () => ({
	...
	default: (props: { session?: Session; id?: number | string; company_id?: number | string }) => {
		...
		return React.createElement(
			'div',
			null,
			`FACTURE_CLIENT_EDIT_FORM_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}:COMPANY_ID=${props?.company_id ?? ''}`,
		);
	},
}));
...
describe('FactureClientEditPage server component', () => {
	...
	it('redirects to FACTURE_CLIENT_LIST when id or company_id is invalid', async () => {
	...
	it('renders form with session, id, and company_id when valid', async () => {
